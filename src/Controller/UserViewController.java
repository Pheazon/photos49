        ...
        else
        {
            ...
            if(obsList.size() != 0 )
            {
               ...
               // td.getResult();
                ...
                for(int i =0; i <obsList.size();i++)
                {
                    if(obsList.get(i).equals(result.get()) || !result.isPresent())
                    {
                        errorAlert.setContentText("Album name already created or canceled");
                        errorAlert.showAndWait();
                        return;
                    }
                }
                obsList.add(result.get());
                listView.setItems(obsList);
                Users.get(currentUserIndex).renameAlbum(index,result.get());
